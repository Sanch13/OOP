...
# где value - числовое значение. В каждом классе следует делать свою проверку на корректность
# значения value:
...
# Затем, используя функции isinstance() и filter(), сформируйте следующие списки из указанных
# объектов:
# lst_positive - все объекты, относящиеся к классу Positive;
# lst_float - все объекты, относящиеся к классу Float.
# P.S. В программе требуется объявить только классы и создать списки. На экран выводить
# ничего не нужно.
...
digits = [PrimeNumber(3), PrimeNumber(1), PrimeNumber(5), FloatPositive(1.5), FloatPositive(9.2),
