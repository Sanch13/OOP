# Подвиг 9 (на повторение). Объявите в программе базовый класс с именем IteratorAttrs для
# перебора всех локальных атрибутов объектов класса. Напомню, что для этого используются два
# магических метода:
# __iter__() - для получения объекта-итератора (в данном случае - это сам объект self)
# __next__() - для перебора локальных атрибутов объекта self
# (используйте для этого словарь __dict__)
...
class IteratorAttrs:
    def __iter__(self):
        for k, v in self.__dict__.items():  # the same --> yield from self.__dict__.items()
            yield k, v                      # the same --> iter(self.__dict__.items())
...
phone = SmartPhone("Luxury", (20, 10), 6)
