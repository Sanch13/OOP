...
#         self.lst.setdefault(eng, [])  # добавление eng(key) - rus(value). "tree" - "дерево"
#         if rus not in self.lst.get(eng):
#             self.lst.get(eng).append(rus)
#
#     def remove(self, eng):
#         del self.lst[
#             eng]  # удаление по ключу eng(key) из лок сво-ва lst[eng] экземпляра класса Translator
#
#     def translate(self,
#                   eng):  # возвращение по ключу eng(key) из лок сво-ва lst[eng] экземпляра класса Translator
#         return self.lst[eng]
#
#
# tr = Translator()
...
# elements = [random.choice(obj_cls)(rnd_num(), rnd_num(), rnd_num(), rnd_num()) for _ in
#             range(217)]  # создается список
...
#         self.a = self.__check_int_float(a)
#         self.b = self.__check_int_float(b)
#         self.c = self.__check_int_float(c)
#
#     @staticmethod
#     def __check_int_float(x):  # Проверка явл. ли х int or float и x >= 0.
#         """return number else None"""
#         return x if isinstance(x, (int, float)) and x >= 0 and x is not True else None
#
#     def is_triangle(self):
#         a, b, c = sides = self.a, self.b, self.c  # переопределение лок. св-в для удобства
#         if None in sides:
...
# tr3 = TriangleChecker(True, True, True)
# print(tr3.__dict__)
...
# class Graph:
#
#     def __init__(self, data: List, is_show: bool = True) -> None:
#         self.data = data.copy()
...
##                "Память: " + '; '.join(f"{obj.name} - {obj.volume}" for obj in self.mem_slots),
