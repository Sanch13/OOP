...
# Каждая строчка списка lst_in должна быть представлена объектом класса MailItem, объекты которого
# создаются командой:
# item = MailItem(mail_from, title, content)
# где mail_from - email отправителя (строка); title - заголовок письма (строка), content -
# содержимое письма (строка). В каждом объекте класса MailItem должны формироваться
# соответствующие локальные атрибуты (с именами: mail_from, title, content).
...
# Вызовите метод:
# mail.receive()
# Отметьте первое и последнее письмо в списке mail.inbox_list, как прочитанное (используйте
# для этого метод set_read). Затем, сформируйте в программе список (глобальный) с именем
# inbox_list_filtered из прочитанных писем, используя стандартную функцию filter()
...
class MailBox:
    ...
    def receive(self):
        lst = [MailItem(*(el.split("; "))) for el in list(map(str.strip, sys.stdin.readlines()))]
        self.inbox_list.extend(lst)
...
print([i.__dict__ for i in mail.inbox_list])
print([i.__dict__ for i in inbox_list_filtered])
# m1 = MailItem('sc_lib@list.ru', 'От Балакирева', 'Успехов в IT!')
# m1.set_read(True)
# print(bool(m1))
# m2 = MailItem('mail@list.ru', 'Выгодное предложение', 'Вам одобрен кредит.')
# print(bool(m2))
