...
# в него прописывают текст программы, в котором заведомо не произойдет исключений,
# отлавливаемых в блоке try. Выполним на практике такой пример.
...
# Здесь фрагмент вычисления w1 // w2 содержит потенциальную ошибку деления на ноль, поэтому его
# следует делать в блоке try. А в блоке else продолжить вычисления,
# где не используются операции деления.
