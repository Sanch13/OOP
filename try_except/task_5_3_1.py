...
# формируемые диапазон [min_length; max_length]); chars - строка из набора символов (хотя бы один
# из них должен присутствовать в проверяемой строке). Если chars - пустая строка, то проверку
# на вхождение символов не делать.
# В самом классе ValidatorString объявите метод:
# def is_valid(self, string): ...
# который проверяет строку string на соответствие критериям: string должна быть строкой, с
# длиной в диапазоне [min_length; max_length] и в string присутствует хотя бы один символ из chars.
...
# значением введенного логина (строки) и ключ 'password' со значением введенного пароля
# (строка). Если хотя бы одного ключа нет, то генерировать исключение командой:
# raise TypeError('в запросе отсутствует логин или пароль')
# В противном случае (если проверка для request прошла), проверять корректность полученного
# формой логина и пароля с помощью валидаторов, указанных в параметрах login_validator и
# password_validator, при создании объекта формы.
# Если логин/пароль введены верно, то в объекте класса LoginForm локальным атрибутам
# _login и _password присвоить соответствующие значения.
...
class ValidatorString:
    ...
    def is_valid(self, string):
        if not self.min_length <= len(string) <= self.max_length or self.chars and \
                not any(char in self.chars for char in string):
            raise ValueError('недопустимая строка')
...
class LoginForm:
    ...
    def form(self, request):
        if not request.get("login") or not request.get("password"):
            raise TypeError('в запросе отсутствует логин или пароль')
        self._login = self.login_validator.is_valid(string=request.get("login"))
        self._password = self.password_validator.is_valid(string=request.get("password"))
